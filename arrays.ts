...
const obterMenorPreco = (lista: number[]) => {
...
const obterMaiorPreco = (lista: number[]) => {
 if (lista.length === 0 || !Array.isArray(lista)) {
   return undefined;
 }
...
const capitalizarNomes = (nomes: string[]) => {
...
// ('Alimentação') => 30
// ('Infantil') => 15
const obterDescontoCategoria = (categoria: string) => {
...
// ([5, 7, 9, 50, 20], 9) => [5, 7, 9]
const obterPrecosLimitadosAoOrcamento = (lista: number[], precoMaximo: number) => {
  if (lista.length === 0 || !Array.isArray(lista)) {
    return undefined;
...
// [10, 30, 5, 15] => 60
const calcularTotalDaCompra = (lista: number[]) => {
  if (lista.length === 0 || !Array.isArray(lista)) {
    return undefined;
  }
