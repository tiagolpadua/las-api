...
function obterPrecosLimitadosAoOrcamento(lista, precoMaximo) {
    ...
    if(Array.isArray(lista) && (lista != 0) ){
        return  menoresPrecos = lista.filter( preco => preco <= precoMaximo)
    }else{
        return undefined;
    }
...
function obterMenorEMaiorPrecos(lista) {
    ...
    const menorMaiorPrecos = [];

    if(!Array.isArray(lista) || (lista.length === 0) ){
        return  undefined;
    }
    const menorPreco = obterMenorPreco(lista);
    const maioPreco = obterMaiorPreco(lista);    
    menorMaiorPrecos[0] = menorPreco;
    menorMaiorPrecos[1] = maioPreco;
   return menorMaiorPrecos;     
...
function capitalizarNomeCompleto(nomeCompleto) {
    return nomeCompleto.replace(/\w\S*/g, function(nomeCompleto){ 
        
        if ( nomeCompleto.charAt(0) < 3){
            return nomeCompleto.charAt(0).toLowerCase();
        }
        return nomeCompleto.charAt(0).toUpperCase() + nomeCompleto.substr(1).toLowerCase();
    });
