...
// ('Alimentação') => 30
// ('Infantil') => 15
function obterDescontoCategoria(produto) {
  const categorias = ["Alimentação", "Infantil"];
  const descontos = [30, 15];
  if (categorias.indexOf(produto) < 0) {
    return 0;
  } else {
    return descontos[categorias.indexOf(produto)];
  }
}
...
function obterMenorEMaiorPrecos(lista) {
  if (Array.isArray(lista) && lista.length > 0) {
    ...
    const saida = [];
    saida.push(lista[0]);
    saida.push(lista[lista.length - 1]);
    return saida;
  } else {
    return undefined;
  }
...
function obterPrecosDentroDoOrcamento(lista, menorValor, maiorValor) {
  ...
  if (Array.isArray(lista) && lista.length > 0) {
    resultado = lista.filter(
      (preco) => preco >= menorValor && preco <= maiorValor
    );
  }
...
function obterDescontoTotal(categoria, cupom) {
    let valorDesconto = obterDescontoCategoria(categoria); //0 ou 15 ou 30
    const listaCupom = ["NULABSSA", "ALURANU"];

    if (valorDesconto === 15 || valorDesconto === 30){
        if (cupom  === listaCupom[0] || cupom === listaCupom[1]){
            return valorDesconto += 10;
        }else {
            return valorDesconto;
        }
    }else if (valorDesconto === 0) {
        return (cupom  === listaCupom[0] || cupom === listaCupom[1]) ? 10 : 0;
    }
}
...
function calcularTotalDaCompraComDescontos(precos, categorias, cupom) {
    if (precos.length !== categorias.length){
        ...
        for (let index = 0; index < categorias.length; index++){
            let desconto = obterDescontoTotal(categorias[index], cupom);
            valorDescontado += precos[index] - (precos[index] * (desconto/100));
        }
...
function capitalizarNomeCompleto(nomeCompleto) {
  ...
  for (index = 0; index < nomeSeparado.length; index++) {
    if (nomeSeparado[index].length < 3) {
      capitalizado.push(nomeSeparado[index]);
    } else {
      nomeSeparado[index] = nomeSeparado[index][0].toUpperCase() + nomeSeparado[index].slice(1).toLowerCase();
      capitalizado.push(nomeSeparado[index]);
    }
  }
