...
function obterMenorPreco(lista) {
    ...
    let menorPreco = lista[aux-1];

    while (aux--){
        if(lista[aux] < menorPreco){
            menorPreco = lista[aux]
        }
    }
    if (!isNaN(menorPreco) === true) {
    return menorPreco;
}
}
...
function obterMaiorPreco(lista) {
    let aux = lista.length;
    let maiorPreco = lista[aux-1];
    while (aux--){
        if(lista[aux] > maiorPreco){
            maiorPreco = lista[aux]
        }
    }
    if (!isNaN(maiorPreco) === true) {
            return maiorPreco;
    }
}
...
function capitalizarNomes(nomes) {
    if ((Array.isArray(nomes) === true) && (!isNaN(nomes) === false)) {

        let nomesAtualizados = nomes.map(nomes => nomes.charAt(0).toUpperCase() + nomes.slice(1).toLowerCase());
...
function obterDescontoCategoria(categoria) {
    ...
    let indCateg = categorias.indexOf(categoria);
	let descontando = descontos[indCateg];
...
function obterPrecosLimitadosAoOrcamento(lista, precoMaximo) {
	...
	if ((!isNaN(precoMaximo) === true) && (!isNaN(lista) === false) && (lista != 0)) {
...
if ((!isNaN(lista) === false)  && (typeof aux != 'string')){
...
function obterMenorEMaiorPrecos(lista) {
	...
	if ((!isNaN(lista) === false) && (lista != 0)) {

	for (let i = 0; i < lista.length; i++) {
		if ((lista[i] === Math.min(...lista))) {
			aux.push(lista[i]);
			} 
		if ((lista[i] === Math.max(...lista))) {
			aux.push(lista[i]);
			} 
		} if (aux != 0){
			return aux; }
