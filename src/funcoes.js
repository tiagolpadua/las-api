...
function validaData(data) { 
    ...
    let dataInput = data.replace("/","-");

   
 
    let dataParaTratar = new Date(dataInput);

    let dataFormatada = dataParaTratar.toLocaleDateString('pt-BR',{timeZone:'UTC'});
   


    console.log("data: ",dataParaTratar.getDate())

    let dia = dataParaTratar.getDay()>0?true:false;

    let mes = dataFormatada.getMonth();

    let ano = dataParaTratar.getFullYear()>1970?true:false;

    //let verificar = dia&&mes&&ano;

   
console.log(dia, " dia: ",dataParaTratar.getDay()," ", mes, " mes: ",dataParaTratar.getMonth(), " ",ano, " ano: ",dataParaTratar.getFullYear(), " Resultado: ",dia&&mes&&ano);
