...
function calculaImposto(preco, categoria) { 
    return categoria == "Alimentação" ? preco = 0 : preco = preco * 0.1;
}
...
function calculaDesconto(preco, categoria, cupom) { 
    return categoria == "Alimentação" && cupom == "NULABSSA" ? preco = preco * 0.5 : preco;
}
...
function truncar(palavra, comprimento = 5) { 
    return palavra.length > comprimento ? `${palavra.substr(palavra, comprimento)}...` : palavra;
}
...
function validaTextoPreenchido(texto) {
    return texto.trim().length === 0 ? undefined : texto.trim(); 
}
