...
function extrairPrimeiroNome(nome) { 
    const identificaEspaco = nome.indexOf(' ');
    return nome.slice(0, identificaEspaco);
  }
...
function calculaDesconto(valor, categoria, cupom) { 
    return categoria === 'Alimentação' && cupom === 'NULABSSA'? valor * 0.5 : valor
    }
...
function truncar(palavra, comprimento = 5) { 
    if (palavra.length <= comprimento) {
      ...
      return palavra.slice(0, comprimento).concat('');
    }
  }
