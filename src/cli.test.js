...
describe("Desejável", () => {
  ...
  test ("Deve emitir erro se informar uma opção inválida.", async () => {
    await expect(processarOpcao("opção-inválida")).rejects.toThrow("Opção inválida: opção-inválida");
  });
  ...
  test ("Deve emitir erro não informar uma opção.", async () => {
    await expect(processarOpcao()).rejects.toThrow("Favor informar uma opção");
  });
