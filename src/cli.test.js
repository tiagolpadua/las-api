...
describe("Desejável", () => {
 ...
 test("Deve listar todos os produtos.", async () => {
  listarProdutos.mockResolvedValue(PRODUTOS_MOCK);
    const saida = await processarOpcao("produtos");
    expect(saida).toEqual(PRODUTOS_MOCK);
  });

  // Crie uma opção e o teste desta opção, que lista os produtos com o preço formatado utilizando
  // o api-service quando é informado argumento 'preco-formatado' na linha de comandos.
  // Utilize PRODUTOS_FORMATADO_MOCK
  // test "Deve listar os produtos com preço formatado."
  test("Deve listar todos os produtos com preço formatado.", async () => {
    listarProdutos.mockResolvedValue(PRODUTOS_MOCK);
      const saida = await processarOpcao("preco-formatado");
      expect(saida).toEqual(PRODUTOS_FORMATADO_MOCK);
    });
  ...
  test("Deve listar todos as categorias.", async () => {
    listarCategorias.mockResolvedValue(CATEGORIAS_MOCK);
      const saida = await processarOpcao("categorias");
      expect(saida).toEqual(CATEGORIAS_MOCK);
    });
  ...
  test("Deve listar os produtos com preço formatado e desconto.", async () => {
    listarProdutos.mockResolvedValue(PRODUTOS_MOCK);     
      const saida = await processarOpcao("descontos");
      expect(saida).toEqual(PRODUTOS_DESCONTO_MOCK);
    });
  // Valide se a opção informada é válida (não esqueça do teste :-)), se não for,
  // emita uma exceção: "Opção ${opcao} inválida.}"
  // test "Deve emitir erro se informar uma opção inválida."
  test("Deve emitir erro se informar uma opção inválida.", async () =>{
    await expect(processarOpcao("errada")).rejects.toThrow("Opção errada inválida.");
  });

  // Valide se foi informada alguma opção (não esqueça do teste :-)), se não for,
  // emita uma exceção: "Informe uma opção."
  // test "Deve emitir erro ao não informar uma opção."
  test("Deve emitir erro ao não informar uma opção.", async() => {
