...
describe("Desejável", () => {
  ...
  test("Deve listar os produtos.", async () => {
    ...
    const opcao = await processarOpcao("produtos");
    //const produtos = await listarProdutos();
    expect(opcao).toEqual(PRODUTOS_MOCK);
  ...
  test("Deve listar os produtos com preço formatado.", async () => {
    ...
    const opcao = await processarOpcao("produtos-formatados");
    const produtos = await listarProdutos();
    expect(opcao).toEqual(produtos);
  ...
  test("Deve listar as categorias.", async () => {
    ...
    const opcao = await processarOpcao("categorias");
    const categorias = await listarCategorias();
    expect(opcao).toEqual(categorias);
  ...
  test("Deve listar os produtos com preço formatado e desconto.", async () => {
    ...
    const opcao = await processarOpcao("descontos");
    const produtos = await listarProdutos();
    expect(opcao).toEqual(produtos);
  ...
  test("Deve emitir erro se informar uma opção inválida.", async () => {
    ...
    await expect(processarOpcao("1")).rejects.toThrow("Opção invalida: 1");
    await expect(processarOpcao("Foo")).rejects.toThrow("Opção invalida: Foo");
    await expect(processarOpcao("Bar")).rejects.toThrow("Opção invalida: Bar");
