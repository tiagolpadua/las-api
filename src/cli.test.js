...
describe("Desejável", () => {
  ...
  test ("Deve listar os produtos com preço formatado.", async () => {
    ...
    const produtosComPrecoFormatado = await processarOpcao("produtos-precos-formatados");
    expect(produtosComPrecoFormatado).toEqual(PRODUTOS_FORMATADO_MOCK);
  ...
  test ("Deve listar os produtos com preço formatado e desconto.", async () => {
    ...
    const produtosComDesconto = await processarOpcao("descontos");
    expect(produtosComDesconto).toEqual(PRODUTOS_DESCONTO_MOCK);
  });
  ...
  test ("Deve emitir erro se informar uma opção inválida.", async () =>{
    const resposta = await processarOpcao("Opção inválida");
    await expect(resposta).rejects.toThrow("Opção inválida");
  });
