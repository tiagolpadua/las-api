const { listaEhInvalida, formataValor, } = require("./arrays");
...
function obterMenorPreco(produtos) {
    ...
    const listaDeValores = produtos.map(prod => prod.preco);
    return produtos[listaDeValores.indexOf(Math.min(...listaDeValores))];
}
...
function obterMaiorPreco(produtos) {
    ...
    const listaDeValores = produtos.map(prod => prod.preco);
    return produtos[listaDeValores.indexOf(Math.max(...listaDeValores))];
}
...
function incluirPrecoFormatado(produto) {
    const copiaProduto  = {...produto};
     copiaProduto.precoFormatado = formataValor(produto.preco, 4);
     return copiaProduto;
...
function obterProdutosLimitadosAoOrcamento(produtos, precoMaximo) {
    ...
    return produtos.filter(prod => prod.preco <= precoMaximo);
}
...
function obterProdutosDentroDoOrcamento(produtos, menorValor, maiorValor) {
    ...
    return produtos.filter(prod => prod.preco >= menorValor && prod.preco <= maiorValor);
}
...
function obterDescontoTotal(categoria, cupom) {
 return cupomEhValido(cupom) ? obterDescontoCategoria(categoria) + cupom.desconto : obterDescontoCategoria(categoria);
}
...
function calcularTotalDaCompraComDescontos(produtos, cupom) {
    ...
    const descontoTotal = produtos.map(prod => prod.preco * (obterDescontoTotal(prod.categoria, cupom) / 100)).reduce((a, b) => a + b, 0);
...
class CarrinhoDeCompras {
    ...
    excluirProduto(indiceDoProduto){
        this.produtos.splice(indiceDoProduto);
    }
    listarProdutos(){
