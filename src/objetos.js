...
function obterMenorPreco(produtos) {
    if (validarLista(produtos)) {
        return undefined;
    }
    let precoMin = produtos[0];
    produtos.filter(function (produto) {
        if (produto.preco < precoMin.preco) {
            precoMin = produto;
        }
    });
    return precoMin;
...
function obterMaiorPreco(produtos) {
    ...
    let precoMax = produtos[0];
    produtos.filter((produto) => {
        if (produtos.preco > precoMax.preco) {
            precoMax = produto;
        }
    });
    return precoMax;
...
function incluirPrecoFormatado(product) {
    let newObj = Object.assign({}, product);
    newObj.precoFormatado = formatarValor(product.preco);
    return newObj;
}
...
function obterDescontoCategoria(nomeCategoria) {
    for (let value of Object.values(CATEGORIAS)) {
        if (value.nome.includes(nomeCategoria)) {
...
function obterProdutosLimitadosAoOrcamento(produtos, precoMaximo) {
    ...
    return produtos.filter(item => item.preco <= precoMaximo);
}
...
function calcularTotalDaCompra(produtos) {
    ...
    return produtos
        .filter(item => ((item.preco), (item.quantidade)))
...
function obterProdutosDentroDoOrcamento(produtos, menorValor, maiorValor) {
    if (validarLista(produtos) || menorValor > maiorValor) {
        return undefined;
    }
    return produtos.
        filter(item => ((item.preco >= menorValor) && (item.preco <= maiorValor)));
}
