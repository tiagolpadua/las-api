...
function obterMenorPreco(produtos) {
  if (listaEhInvalida(produtos)) {
    ...
    const listaMenorValor = produtos.map((prod) => prod.preco);
    return produtos[listaMenorValor.indexOf(Math.min(...listaMenorValor))];
  }
}
...
// Crie uma função que receba um produto e retorna uma cópia deste produto
//incluindo uma nova proprieade
...
function incluirPrecoFormatado(produto) {
  const produtoComPrecoFormatado = { ...produto };
  produtoComPrecoFormatado.precoFormatado = formatarValor(produto.preco);

  return produtoComPrecoFormatado;
}
...
function obterDescontoCategoria(nomeCategoria) {
  ...
  const desconto = CATEGORIAS.map((categ) => {
    if (categ.nome === nomeCategoria) {
      descontoCategoria = categ.desconto;
    }
  });
...
function obterProdutosLimitadosAoOrcamento(produtos, precoMaximo) {
  if (listaEhInvalida(produtos)) {
    ...
    const listaProd = [];
    const prodNoOrcamento = produtos.map((prod) => {
      if (prod.preco <= precoMaximo) {
        listaProd.push(prod);
      }
    });
...
function calcularTotalDaCompra(produtos) {
  if (listaEhInvalida(produtos)) {
    ...
    const listaDePrecos = [];
    const verificaPrecos = produtos.map((prod) => {
      listaDePrecos.push(prod.preco * prod.quantidade);
    });
    return listaDePrecos.reduce((acum, atual) => atual + acum);
  }
}
...
function obterProdutosDentroDoOrcamento(produtos, menorValor, maiorValor) {
  if (listaEhInvalida(produtos) || menorValor > maiorValor) {
    ...
    const prodNoOrcamento = produtos.map((prod) => {
      if (
        prod.preco >= menorValor ||
        obterProdutosLimitadosAoOrcamento(produtos, maiorValor) <= maiorValor
      ) {
        listaProd.push(prod);
      }
    });
