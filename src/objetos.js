...
function obterProdutosDentroDoOrcamento(produtos, menorValor, maiorValor) {
    ...
    if (menorValor < maiorValor) {
        return produtos.filter(x => x.preco >= menorValor && x.preco <= maiorValor);
    }
...
function calcularTotalDaCompraComDescontos(produtos, cupom) {
    ...
    for (let i = 0; i < produtos.length; i++) {
        totalDaCompra += (produtos[i].preco * produtos[i].quantidade) - ((produtos[i].preco * produtos[i].quantidade) * (obterDescontoTotal(produtos[i].categoria, cupom) / 100));
    }
    return totalDaCompra;
}
