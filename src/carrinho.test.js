...
describe("Desafio", () => {
  ...
  test("Deve exibir as opções.", () => {
    console.log = jest.fn();
    exibirOpcoes();
    expect(console.log.mock.calls).toEqual([["Escolha uma Opção:"], ["1 - Liste os produtos"], ["2 - Inclua um produto no carrinho"], ["3 - Visualize o carrinho"], ["4 - Finalizar a compra"], ["x - Saia do sistema"]]);
  });
  ...
  test("Deve incluir os produtos ao digitar opção 2.", async () => {
    listarProdutos.mockResolvedValue(PRODUTOS_MOCK);
    console.log = jest.fn();
    console.table = jest.fn();
