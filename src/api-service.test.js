...
describe("Essencial", () => {
  ...
  it("Deve ter uma função que lista as categorias", async () => {
    fetch.mockResolvedValue({
      status: 200,
  ...
  it("Deve ter uma função que lista os cupons válidos", async () => {
    fetch.mockResolvedValue({
      status: 200,
      json: () => Promise.resolve(CUPONS_MOCK),
    });
    const produtos = await listarCupons();
  ...
  it("Deve tratar erros 404.", async () => {
    fetch.mockResolvedValue({
      status: 404,
