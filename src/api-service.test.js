...
describe("Essencial", () => {
  ...
  test("Deve ter uma função que lista os categorias", async () => {
    fetch.mockResolvedValue({
      status: 200,
  ...
  test("Deve ter uma função que lista os cupons válidos", async () => {
    ...
    const cupons = await listarCupomnsValidos();
  ...
  test("Deve tratar erros 404.", async () => {
    ...
    await expect(listarCupomnsValidos()).rejects.toThrow("Not found: 400");
  });
});
