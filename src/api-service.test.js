...
//Importando o módulo que será testado
...
describe("Essencial", () => {
  ...
  test("Deve tratar erros 404.", async () => {
    ...
    await expect(listarProdutos()).rejects.toThrow(`${response.statusText}: ${response.status}`);
    await expect(listarCategorias()).rejects.toThrow(`${response.statusText}: ${response.status}`);
    await expect(listarCupons()).rejects.toThrow(`${response.statusText}: ${response.status}`);
  });
});
